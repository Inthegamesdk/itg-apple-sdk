...
public struct ITGPlayerViewControllerSwiftUI: UIViewControllerRepresentable {
    ...
    public func updateUIViewController(_ uiViewController: ITGPlayerViewController, context: Context) {
        if context.coordinator != self.makeCoordinator() {
            context.coordinator.itgPlayerViewController?.reloadChannel(channelSlug: channelSlug, virtualChannels: virtualChannels, accountId: accountId, environment: environment, foreignId: foreignId, vars: vars, playerAdapter: playerAdapter, shouldResetOverlayUser: false, showLogs: showLogs)
        }
    }
